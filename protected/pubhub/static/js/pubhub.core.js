var pubhub = pubhub || function ($) {

    /**
     * Contains the modules namespace
     * @type object
     */
    var modules = {};
    ...
    var module = function (id, moduleFunction) {
        ...
        instance.id = 'pubhub.modules.' + _cutModulePrefix(id);
     ...
     * or classes.
     *
     * For the module pubhub.modules.ui.modal you can search:
     *
     * require('ui.modal');
     * require('modules.ui.modal');
     * require('pubhub.modules.ui.modal');
     *
     * @param {type} moduleId
    ...
    var resolveNameSpace = function (typePath, init) {
        try {
            //cut pubhub.modules prefix if present
    ...
    var _cutModulePrefix = function (value) {
        return _cutPrefix(_cutPrefix(value, 'pubhub.'), 'modules.');
    };

    /**
     * Cuts a prefix from a string, this is already available in pubhub.util but
    ...
    $(function() {
        ...
        $.each(moduleArray, function (i, module) {
        ...
        pubhub.initialized = true;
        event.trigger('pubhub:ready');
        $(document).trigger('pubhub:ready', [false, pubhub]);
    });

    var initModule = function (module) {
        var log = require('log');
        event.trigger('pubhub:beforeInitModule', module);
        if (module.init) {
            try {
        ...
        event.trigger('pubhub:afterInitModule', module);
    ...
    event.on('pubhub:modules:client:pjax:success', function (evt) {
        ...
        event.trigger('pubhub:ready');
        $(document).trigger('pubhub:ready', [true, pubhub]);
    }).on('pubhub:modules:client:pjax:beforeSend', function (evt) {
        unloaded = [];
        $.each(moduleArray, function (i, module) {
            if (module.unload) {
                module.unload();
